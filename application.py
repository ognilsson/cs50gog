...
def activities():
    ...
    if request.method == "POST":
        ...
        for index in range(0,len(preferences)):
            activities.append(db.execute("SELECT category_name,activity_name from activities JOIN categories ON activities.category_id = categories.category_id where categories.category_id=?;",preferences[index]["preference_id"]))
        ...
        return render_template("activities.html",len=length,activities=activityImagePaths,activityTitles=activityTitles,categoryTitles=activityCategoryTitles,categories=len(activityCategoryTitles))
